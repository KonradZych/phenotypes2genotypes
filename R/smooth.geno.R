...
smooth.geno <- function(cross, windowSize=1, chr, population,map=c("genetic","physical"), fix.positions=FALSE, verbose=FALSE,...){
  ...
  cross <- replace.map(cross, est.map(cross, offset=0, ...))
...
smooth.genoSub.internal <- function(geno, windowSize, oldMarkers,verbose){
  if(is.null(dim(geno$data))) stop("Error - smooth.genoSub.internal - Genotype data is NULL")
  if(ncol(geno$data) > windowSize){
    old_genotype <- geno$data
    old_genotype[which(is.na(old_genotype))]<- 0
    genotype <- t(apply(old_genotype, 1, smooth.genoRow.internal, windowSize))
    ...
    stop("Error - smooth.genoSub.internal: ncol(geno$data) < windowSize")
  }
  invisible(geno)
}
...
#
############################################################################################################
#TODO: REWRITE THE IF OCNDITONS FOR THIS FUNCTION !!!!!
smooth.genoRow.internal <- function(genoRow,windowSize){
  if(length(table(genoRow))>1){
    ...
    if(length(wrongMarkers)>0) genoRow[wrongMarkers]<-NA
  }
  invisible(genoRow)
}
